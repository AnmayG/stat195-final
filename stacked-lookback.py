...
df = pd.read_csv(CSV_FILE, parse_dates=["Date"]).sort_values(
...
plt.plot(df["Date"][:split_idx], train_raw,
...
len_tr = len(y_train_max)
len_te = len(y_test_max)
...
param = {"C": [0.1, 0.3, 1, 3, 10, 100], "gamma": [
    0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 2], "epsilon": [0.001, 0.005, 0.01, 0.05, 0.1, 0.2]}
...
def evaluate_by_horizon():
    ...
    models = [f"LSTM{h}L{SEQ_SET[i]}" for i,
              h in enumerate(HIDDEN_SET)] + ["STACK"]
    ...
    for h in horizons:
        ...
        for name, pred_scaled in zip(models, predictions):
            p_usd = scaler.inverse_transform(
                pred_scaled.reshape(-1, 1)).squeeze()
            y_h, f_h = truth_usd[h:], p_usd[:-h]
...
def save_plot(name, train_pred_scaled, test_pred_scaled,
    ...
    # inverse-transform
    tr_usd = scaler.inverse_transform(
        train_pred_scaled.reshape(-1, 1)).squeeze()
    te_usd = scaler.inverse_transform(
        test_pred_scaled.reshape(-1, 1)).squeeze()
